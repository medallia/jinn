...
cpus=$(bc <<< "scale=1; $(grep -c processor /proc/cpuinfo) - 0.1")
echo "CPU: ${cpus}"
mem=$(bc <<< "scale=0; total=$(grep MemTotal /proc/meminfo | awk '{print $2}')/1024; if (total >= 2048) {total - $reservation} else {total/2}")
echo "MEM: ${mem}"
